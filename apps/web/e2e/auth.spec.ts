...
async function authenticate(context:BrowserContext){const l=await launcher();const page=await context.newPage();await page.goto(l.url);await expect(page.getByRole("navigation", { name: "System" })).toBeVisible();return{page,token:l.token,origin:l.url};}
