...
let home:string,db:Db,built:BuiltServer;let clock=new Date("2026-09-03T12:00:00Z");const apiOrigin="http://127.0.0.1:4276";
test.beforeAll(async()=>{home=mkdtempSync(join(tmpdir(),"acp-e2e-"));const config=loadConfig({AGENT_PLANE_HOME:home});config.api.port=4276;db=openDb(config.dbPath);built=buildServer({config,db,now:()=>clock});built.registry.init();await built.app.listen({host:"127.0.0.1",port:4276});});
...
async function authenticate(context:BrowserContext){const l=await launcher();const page=await context.newPage();await page.goto(l.url);await expect(page.getByText("Operator workspace", { exact: true })).toBeVisible();return{page,token:l.token,origin:l.url};}
