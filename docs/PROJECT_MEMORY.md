...
- **The §7.2 bar is a frequency** of 2 of 5 on the gate outcome or a risk cross, with fewer than 5 judged runs counting as INCOMPLETE. It is implemented in `apps/api/src/modules/decision-frequency.ts`, runs from `eval.yml` through `decision:injection-frequency` (manual dispatch only since K19i), and is pinned by `decision-frequency.test.ts`.
...
## 2026-09-23 — M16 K19i: floors decide, the judge moves offline (SHIPPED; shadow only; judge survey re-run BLOCKED on credential)
...
- **The 5-run §7.2 judge suites are manual dispatch only.** They fail by construction and no longer gate anything. The gate's injection check is the deterministic carrier test, which runs per PR.
