...
## 2026-09-23 — M16 K19i: floors decide, the judge moves offline (SHIPPED; shadow only; judge survey re-run BLOCKED: 401 on the operator key)
...
- **Real traffic, n = 21** (the operator DB, 2026-09-15): the floors prompt on 15. Nine of those are MCP tools no floor understands (`opaque`, a policy choice). Six are shell commands listing `~`. Watch the MCP share in the prompt rate.
- **§7.2 is unchanged and still an owner decision.** The bar, its nightly run and the activation precondition are as K19h left them. What K19i adds is a fact: the gate no longer reads the judge, and appended carrier text cannot remove a floor. That is pinned per PR, with no credential.
- **Live judge on 2026-09-24:** `~/.agent-plane/personal/anthropic.key` must hold only the raw key, because a `NAME=` prefix gives a 401. With the key fixed, the account returned 400 "credit balance is too low". The survey re-run and discovery both judged 0. Add credit before expecting floor candidates.
